...
export class FlutterwaveGateway implements PaymentGateway {
  ...
  async initialize(input: PaymentInitRequest): Promise<PaymentInitResult> {
    const response = await fetch(`${this.baseUrl}/payments`, {
      ...
      body: JSON.stringify({
        ...
        customer: { email: input.email, phonenumber: input.phone, name: input.fullName }
      })
    });
  ...
  async verify(reference: string): Promise<PaymentVerificationResult> {
    ...
    const body = await response.json();
    if (!response.ok || body.status !== "success") throw new Error(body.message || "Flutterwave verification failed");
    return {
      reference,
  ...
  verifyWebhookSignature(_rawBody: string, signature?: string): boolean {
    ...
    return Boolean(secretHash && signature && crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(secretHash)));
  }
